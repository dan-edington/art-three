...
function artwork(p5: P5): void {
  ...
  const nD = new Date().getTime() * Math.random();
  const nO = new Date().getTime() * Math.random();
  let xVariance = 1;
  let yVariance = 1;
  let chance = 1;
  let stopped = false;
  ...
  p5.draw = function () {
    ...
    const yCount = 15;
    chance -= 0.01;
    xVariance += p5.random(-0.25, 0.25) * 2;
    yVariance += p5.random(-0.3, 0.3) * 2;
    const padding = 75;
