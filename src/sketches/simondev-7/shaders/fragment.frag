...
void main() {
  ...
  float line = smoothstep(0.004, 0.005, abs(vPosition.y - mix(-0.5, 0.0, value1)));
  baseColor = mix(color3, baseColor, line);
 ...
 // Fragment shader colours
  if(vPosition.y > 0.0) {
    ...
    float line2 = smoothstep(0.004, 0.005, abs(vPosition.y - mix(0.0, 0.5, t)));
    baseColor = mix(color3, baseColor, line2);
  }

  // Dividing line
  float dividingLine = smoothstep(0.004, 0.005, abs(vPosition.y));
  baseColor = mix(vec3(0.0, 0.0, 0.0), baseColor, dividingLine);
